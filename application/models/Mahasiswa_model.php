<?php 

/**
 * 
 */
class Mahasiswa_model extends CI_Model
{
	

	var $table = 'mahasiswa'; //nama tabel dari database
    var $column_order = array(null, 'nim','namaLengkap','jalurMasuk'); //field yang ada di table user
    var $column_search = array('nim','namaLengkap','jalurMasuk'); //field yang diizin untuk pencarian 
    var $order = array('nim' => 'asc'); // default order 
 

	function getWhere($Value)
	{
		$this->db->where($Value);
		return $this->db->get('mahasiswa');
	}
	public function getId($id)
	{
		$this->db->where('nim',$id);
		return $this->db->get('mahasiswa');
	}
	public function insert($value)
	{
		$this->db->insert('mahasiswa', $value);
	}
	public function nimSearch($value)
	{
		$sql = "SELECT nim FROM mahasiswa WHERE nim LIKE '%".$value."%' ";
		return $this->db->query($sql)->result();
	}

	public function delete($value)
	{
		$this->db->where('nim', $value);
		$this->db->delete('mahasiswa');
	}

	public function update($id,$data)
	{
		$this->db->where('nim', $id);
		$this->db->update('mahasiswa', $data);
	}
	public function getAll($value='')
	{
		return $this->db->get('mahasiswa');
	}
	function get_enum_values( $table, $field )
	{
	    $type = $this->db->query( "SHOW COLUMNS FROM {$table} WHERE Field = '{$field}'" )->row( 0 )->Type;
	    preg_match("/^enum\(\'(.*)\'\)$/", $type, $matches);
	    $enum = explode("','", $matches[1]);
	    return $enum;
	}
	 private function _get_datatables_query()
    {
         
        $this->db->from($this->table);
 
        $i = 0;
     
        foreach ($this->column_search as $item) // looping awal
        {
            if($_POST['search']['value']) // jika datatable mengirimkan pencarian dengan metode POST
            {
                 
                if($i===0) // looping awal
                {
                    $this->db->group_start(); 
                    $this->db->like($item, $_POST['search']['value']);
                }
                else
                {
                    $this->db->or_like($item, $_POST['search']['value']);
                }
 
                if(count($this->column_search) - 1 == $i) 
                    $this->db->group_end(); 
            }
            $i++;
        }
         
        if(isset($_POST['order'])) 
        {
            $this->db->order_by($this->column_order[$_POST['order']['0']['column']], $_POST['order']['0']['dir']);
        } 
        else if(isset($this->order))
        {
            $order = $this->order;
            $this->db->order_by(key($order), $order[key($order)]);
        }
    }
 
    function get_datatables()
    {
        $this->_get_datatables_query();
        if($_POST['length'] != -1)
        $this->db->limit($_POST['length'], $_POST['start']);
        $query = $this->db->get();
        return $query->result();
    }
 
    function count_filtered()
    {
        $this->_get_datatables_query();
        $query = $this->db->get();
        return $query->num_rows();
    }
 
    public function count_all()
    {
        $this->db->from($this->table);
        return $this->db->count_all_results();
    }
	
}





?>